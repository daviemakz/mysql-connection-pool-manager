...
class PoolFunctions {
  ...
  // FUNCTION: MyMySQLL query emit warning
  emitQueryError(error) {
    if(typeof process !== 'undefined' && typeof process.emitWarning !==
      'undefined') {
      process.emitWarning(Utility.inspect(error, false, null)
        , 'unhandledRejection');
    } else {
      console.warn(Utility.inspect(error, false, null));
    }
  }
