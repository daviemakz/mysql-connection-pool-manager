...
class Core extends PoolFunctions {
    ...
    this.poolStorage.getConnection(function(err, con) {
      ...
      if(err) {
        if(this.options.errorLimit > errorTracking) {
          // Console log
          console.warn(
            `Initializing connection failed. MORE INFO: ${err} Retrying...`
          );
          // Track the failiure
          errorTracking++;
  ...
  execute() {
    ...
    if(!(connection)) {
      if(typeof(returnCallback) === 'function') {
        returnCallback([]
          , `Error getting connection object! MORE INFO: ${errorMessage}`);
        return;
      } else {
        return;
      }
  ...
  query(command, callback) {
    ...
    AS.series([
      ...
      if(error) {
        console.warn(
          `Unknown error in async series stack. MORE INFO: ${error}`);
      }
    });
  }
