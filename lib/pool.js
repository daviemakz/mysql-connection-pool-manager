...
class Core extends PoolFunctions {
  ...
  assignEventListeners() {
    ...
    this.poolStorage.on('connection', (connection) => {
      ...
      typeof(this.options.onConnectionConnect) === 'function' && this.options
    ...
    // Assign `onConnectionAcquire` if defined
    typeof(this.options.onConnectionAcquire) === 'function' && this.poolStorage
      .on(
        'acquire', (connection) => {
          this.options.onConnectionAcquire(connection);
        });
    // Assign `onConnectionEnqueue` if defined
    typeof(this.options.onConnectionEnqueue) === 'function' && this.poolStorage
      .on(
        'enqueue', () => {
          this.options.onConnectionEnqueue();
        });
    // Assign `onConnectionRelease` if defined
    typeof(this.options.onConnectionRelease) === 'function' && this.poolStorage
      .on(
        'release', (connection) => {
      ...
      if(err) {
        if(this.options.errorLimit > scope.errorTracking) {
          ...
          if(err.code === 'POOL_CLOSED') {
            ...
            scope.errorMessage = {
              internalMsg: `Query limit reached, try increasing this value! MORE INFO: ${JSON.stringify(err)}`
              , err: err
            };
          ...
          // Assign error message
          scope.errorMessage = {
            internalMsg: `Initializing connection failed. MORE INFO: ${JSON.stringify(err)}`
            , err: err
          };
          // Callback a null object
          return cb();
        ...
        this.poolConnectionList[this.currentSessionId()][con.threadId] =
          this.getTimestamp();
        // Assign connection
  ...
  query(command, callback) {
    ...
    setTimeout((scope) => {
      Async.series([
        ...
        if(error) {
          console.warn(
            `Unknown error in async series stack. MORE INFO: ${JSON.stringify(error)}`
          );
        }
      });
    }, 0, scope);
